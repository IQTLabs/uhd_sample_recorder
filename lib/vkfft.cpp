...
int64_t init_vkfft(size_t batches, size_t sample_id, size_t nfft) {
  ...
  for (uint64_t i = 0; i < kVkNumBuf; ++i) {
    vkConfiguration.bufferSize[i] =
        (uint64_t)sizeof(std::complex<float>) * buffer_size_f;
  }

  vkConfiguration.bufferNum = kVkNumBuf;

  vkConfiguration.buffer = (VkBuffer *)aligned_alloc(
      sizeof(std::complex<float>), kVkNumBuf * sizeof(VkBuffer));
  if (!vkConfiguration.buffer)
    return VKFFT_ERROR_MALLOC_FAILED;
  vkBufferDeviceMemory = (VkDeviceMemory *)aligned_alloc(
      sizeof(std::complex<float>), kVkNumBuf * sizeof(VkDeviceMemory));
...
void vkfft_specgram_offload(arma::cx_fmat &Pw_in, arma::cx_fmat &Pw) {
  ...
  for (arma::uword k = 0; k < Pw_in.n_cols;
    ...
    transferDataFromCPU(&vkGPU, ((char *)Pw_in.memptr()) + offset,
                        &vkConfiguration.buffer[0], buffer_size);
    performVulkanFFT(&vkGPU, &vkApp, &vkLaunchParams, -1, 1);
    transferDataToCPU(&vkGPU, ((char *)Pw.memptr()) + offset,
                      &vkConfiguration.buffer[0], buffer_size);
  }
}
